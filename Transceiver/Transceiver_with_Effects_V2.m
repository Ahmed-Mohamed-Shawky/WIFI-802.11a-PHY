...
%% Channel Paramters
Max_Delay_Spread = 100; % ns
%% CFO Paramters
Ratio = 50; %% (0-100)% CFO Ratio Effect
%% STO Paramters
Min_STO_Samples = 100;
Max_STO_Samples = 200;
...
% 'Channel+CFO+Noise' , 'Paket Shift' , 'Paket Shift+Noise' , 
% 'Paket Shift+CFO+Noise' , 'Paket Shift+Channel+Noise' , 'All'
% SimulationType = 'Paket Shift+Channel+Noise';
% SimulationType = 'Channel+CFO+Noise';
SimulationType = 'Channel+CFO+Noise';
SimulateMode = 1;   % 0 -> One Simulation , 1 -> BER Simulation
% Effects_DebugMode = 1;
...
ConstlationPlot = 0; % Turn Ploting On or Off
...
RX_State = struct("DebugMode",RX_DebugMode, ...
...
% Receiver = IEEE802_11a_Receiver(Wifi_Output);
...
switch SimulationType
    ...
    case 'Channel'
        Effects.add_Channel(Max_Delay_Spread,'Rayleigh'); % or Racian
    case 'Channel+Noise'
...
% ZF_RX_data = General_Receiver_Function(Effects.Waveform,RX_State);
% 
...
Effects.DebugMode = 0; % Enable Effects Debug Mode
...
for dataRateIndex = 1:length(DataRate)
    ...
    for snr_index = 1:length(SNR)
        ...
        for I = 1:Iterations
            ...
            Transmitter.DATARATE = DataRate(dataRateIndex);
            TranmitterOutput = Transmitter.GenerateWaveform(data_hex);
            % Effects.Waveform =  TranmitterOutput.waveform;
            Effects.TransmitterOutput =  TranmitterOutput;
            %% Adding Efficts
            switch SimulationType
                case 'Noise'
                    Effects.add_Noise(SNR(snr_index));
                case 'Channel'
                    Effects.add_Channel(Max_Delay_Spread,'Rayleigh'); % or Racian
                case 'Channel+Noise'
                    Effects.add_Channel(Max_Delay_Spread,'Rayleigh');
            ...
            %% Receiver Block
            try
                RX_data = General_Receiver_Function(Effects.TransmitterOutput.waveform,RX_State);
                % Claculte BER
